...
class PantallaInicio extends React.Component {
  ...
  Entrar(){
    if(!!this.state.usuario && !!this.state.contra){
      fetch(
        'htttps://marketudb.000webhostapp.com/apiusuario.php?comando=autenticar&usuario='+this.state.usuario+'&contraseña='+this.state.contrasena, {
          method: 'GET'
        })
  ...
  render(){
    return(
      <View style={{flex:1}}>
        ...
        />
        <Text style={{fontSize:18, textAlign:'center', height:40, marginTop:10, backgroundColor:'lightgray', textAlignVertical:'center', borderRadius:10, marginLeft:10, marginRight: 10}}>
...
class PaginaAgregar extends React.Component {
  ...
  Guardar(){
    fetch('htttps://marketudb.000webhostapp.com/api.php?comando=agregar&nombre='+this.state.nombre
    +'&descripcion='+this.state.descripcion
    +'&cantidad='+this.state.cantidad
    +'&preciodecosto='+this.state.preciodecosto
    +'&preciodeventa='+this.state.preciodeventa
